...
class ProductosController {
  ...
  static getById = async (req: Request, resp: Response) => {
    try {
      const Cod_Producto = parseInt(req.params["id"]);
      ...
      try {
        producto = await productosRepo.findOneOrFail({
          where: { Cod_Producto, estado: true },
        });
      } catch (error) {
        return resp
          .status(404)
  ...
  static add = async (req: Request, resp: Response) => {
    try {
      //DESTRUCTURING
      const { Cod_Producto, nombre, precio, stock, fechaIngreso } = req.body;
      ...
      const productosRepo = AppDataSource.getRepository(Producto);
      const pro = await productosRepo.findOne({ where: { Cod_Producto } });
  ...
  static update = async (req: Request, resp: Response) => {
    ...
    let pro: Producto;
    try {
      pro = await productosRepo.findOneOrFail({ where: { Cod_Producto } });
    } catch (error) {
    ...
    try {
      ...
      try {
        pro = await productosRepo.findOneOrFail({
          where: { Cod_Producto: Cod_Producto, estado: true },
        });
      } catch (error) {
        return resp
          .status(404)
